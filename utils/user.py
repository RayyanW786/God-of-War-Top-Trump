...
class UserUtils(object):
    ...
    def register_checks(self, username: str, password: str) -> bool:
        flags = [False, False, False, False, False]  # should be True True True True True, order being digit check, uppercase check, lowercase check ratio check, length check
        if len(password) >= 8:
            flags[4] = True
